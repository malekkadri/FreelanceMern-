  ...
  const certificate = await Certificate.findOne({to : req.params.person , type : 'pro' , amount : 100 })
    if(!certificate){
      const newCertificate = new Certificate({
        ...
        pic : '' ,
        amount : 100 ,
        text : 'Congratulations on reaching 100 dollar milestone on Khadamni' ,
        onDate : Date.now()
      })
    ...
    const certificate = await Certificate.findOne({to : req.params.person , type : 'pro' , amount : 1000 })
      if(!certificate){
        const newCertificate = new Certificate({
          ...
          pic : '' ,
          amount : 1000 ,
          text : 'Congratulations on reaching 1000 dollar milestone on Khadamni' ,
          onDate : Date.now()
        })
        await newCertificate.save()
      }
      ...
      const certificate = await Certificate.findOne({to : req.params.person , type : 'pro' , amount : 5000 })
        if(!certificate){
          const newCertificate = new Certificate({
            ...
            pic : '' ,
            amount : 5000 ,
            text : 'Congratulations on reaching 5000 dollar milestone on Khadamni' ,
            onDate : Date.now()
          })
          await newCertificate.save()
        }
      ...
      const certificate = await Certificate.findOne({to : req.params.person , type : 'pro' , amount : 10000 })
        if(!certificate){
          const newCertificate = new Certificate({
            ...
            pic : '' ,
            amount : 10000 ,
            text : 'Congratulations on reaching 10000 dollar milestone on Khadamni' ,
            onDate : Date.now()
          })
          await newCertificate.save()
        }
      ...
      const certificate = await Certificate.findOne({to : req.params.person , type : 'pro' , amount : 50000 })
        if(!certificate){
          const newCertificate = new Certificate({
            ...
            pic : '' ,
            amount : 50000 ,
            text : 'Congratulations on reaching 50000 dollar milestone on Khadamni' ,
            onDate : Date.now()
          })
          await newCertificate.save()
        }
